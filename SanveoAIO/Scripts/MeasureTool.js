(function () {
    ...
    Autodesk.Viewing.Extensions.Measure.MeasureTool = function (viewer, options, sharedMeasureConfig, snapper) {
        ...
        function getPreviewsPick() {
            ...
            goometryvertices = _currentMeasurement.getPick(index);
            if (goometryvertices.id != ids) {
                ids = goometryvertices.id;
                if (goometryvertices.geomVertex != null) {
                    coordinatesx.push(goometryvertices.geomVertex.x.toFixed(2));
                    coordinatesy.push(goometryvertices.geomVertex.y.toFixed(2));
                }
            }
            return _currentMeasurement.getPick(index);
        }
        ...
        this.deactivate = function () {
            ...
            $("#CordsTable tbody").html('');
            var tabledata = '';
            if (coordinatesx.length > 0) {
                for (var i = 1; i <= coordinatesx.length; i++) {
                    tabledata += '<tr><td>Point ' + i + '</td><td>' + coordinatesx[i - 1] + '</td><td>' + coordinatesy[i - 1] + '</td></tr>';
                }
                $("#CordsTable tbody").html(tabledata);
                $("#cordsx").text(coordinatesx.join(","));
                $("#cordsy").text(coordinatesy.join(","));
                $("#CordsWindow").data("kendoWindow").open();
            }
            var dataresult;// = savecoordinates(coordinatesx, coordinatesy);       
            if (dataresult != "") {
                console.log("insifer");
                coordinatesx = [];
                coordinatesy = [];
        ...
        this.getArea = function (measurement) {
            ...
            if (_viewer.model) {
                ...
                var area = Autodesk.Viewing.Private.convertUnits(_viewer.model.getUnitString(), _sharedMeasureConfig.units, _sharedMeasureConfig.calibrationFactor, measurement.area, 'square');
        ...
        function snapToFirstPick(currentPick, forceSnap) {
            if (_currentMeasurement.hasPick(1) && _activePoint > 3 && !_currentMeasurement.closedArea) {
                ...
                var firstPickPosition = MeasureCommon.project(firstPickPoint, _viewer);
                var currentPickPoint = MeasureCommon.getSnapResultPosition(currentPick, _viewer);
                var currentPickPosition = MeasureCommon.project(currentPickPoint, _viewer);
        ...
        this.correctPickPosition = function () {
            ...
            var corrected = MeasureCommon.correctPerpendicularPicks(passive, active, _viewer, _snapper);
            if (!corrected) {
                ...
                if (id !== active.id && _currentMeasurement.hasPick(id)) {
                    ...
                    MeasureCommon.correctPerpendicularPicks(nextPick, active, _viewer, _snapper);
                }
            }
