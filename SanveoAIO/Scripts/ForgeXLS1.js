...
var ForgeXLS = {
    ...
    downloadXLSX: function (c, urn, token, status, loader, excel) {
        var fileName = c + '.xlsx';

        //decodeURIComponent(atob(urn).replace(/^.*[\\\/]/, '')) + '.xlsx';
        ...
        if (status) {
            /// status(false, 'SanveoInspire ' + fileName);
            //status(false, 'Reading Model information....');
        }
        ...
        this.prepareTables(urn, token, function (tables) {
            ...
            loader.hide();
            excel.show();
            // if (status) status(true, 'Downloading SanveoInspireFiles...');
        })
    },
